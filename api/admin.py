...
async def get_admin_stats(current_admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    """Get platform-wide statistics including meetings and errors."""
    ...
    # Count meetings (stored in Email meta_data or audit logs)
    # This is an estimate based on email metadata containing 'meeting' details
    total_meetings = db.query(Email).filter(Email.meta_data.op('->>')('has_meeting') == 'true').count()
    
    # Count errors from Audit Log
...
async def list_users(current_admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    """List all registered users with their basic info."""
    users = db.query(User).order_by(User.id.asc()).all()
    
    # In a real app, we might join with counts of their actions
    # But for now, returning simple list
    return [
        {
            "id": u.id,
            "email": u.email,
...
async def get_user_detail(user_id: int, current_admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    """Get detailed info about a specific user with RFI stats."""
    ...
    # Calculate RFI specific stats from Audit Logs or other tables
...
async def get_audit_logs(limit: int = 100, current_admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    """Get latest audit logs with user information."""
